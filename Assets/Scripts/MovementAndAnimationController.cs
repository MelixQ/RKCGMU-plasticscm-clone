...
public class MovementAndAnimationController : MonoBehaviour
{
    ...
    [SerializeField] private GameObject _playerFeetObject;
    ...
    private IEnumerator MoveTowardsDestination(Vector3 destination)
    {
        ...
        while (Vector3.Distance(transform.position, destination) > .5f)
        {
            ...
            if (Physics.Raycast(_playerFeetObject.transform.position, positionToLookAt, out var hit, .5f))
                break;
    ...
    public void ToFinishSceneState() => _gameEnded = true;
